...
struct narray combinePermutations(struct narray firstPermutation,
     ...
     for (unsigned int i = 0; i < N; i++) {
         result.arr[i] = secondPermutation.arr[firstPermutation.arr[i]];
     }
     return result;
}
...
unsigned int isBottom(struct fractions probs) {
    ...
    if (WEAK_SECURITY == 2) {
        ...
        bottom = (probs.frac[0].num > 0 || probs.frac[3].num > 0) && (probs.frac[1].num > 0 || probs.frac[2].num > 0); // MODIFIED
    }
    return bottom;
}
    ...
    if (isValid(s)) { // Non-valid states cannot be final.
        ...
        for (unsigned int i = 0; i < NUMBER_POSSIBLE_SEQUENCES; i++) {
            if (!done && isStillPossible(s.seq[i].probs)) {
                ...
                unsigned int deciding = WEAK_SECURITY == 2 ? (s.seq[i].probs.frac[1].num > 0) : (s.seq[i].probs.frac[1].num > 0 || s.seq[i].probs.frac[2].num > 0); // MODIFIED
...
void checkTransitivityOfPermutation(unsigned int permutationSet[MAX_PERM_SET_SIZE][N],
    ...
    if (FORCE_RANDOM_CUTS && !onlyPerm) {
        // MODIFIED
        // Random cut exists iff the size of a permutation set is less than or equal to N.
        assume (permSetSize <= N);

        // check the permutation set whether it contains a permutation \sigma that consists of exactly 1 loop
        unsigned int hasSingleLoop = 0;
        unsigned int isSingleLoopPerm[MAX_PERM_SET_SIZE] = { 0 };

        for (unsigned int i = 0; i < MAX_PERM_SET_SIZE; i++) {
            if (i < permSetSize) {
                unsigned int staysFix[N] = { 0 };
                unsigned int isIdentityPerm = 1;
                unsigned int firstNotFix = 0;
                for (unsigned int j = 0; j < N; j++) {
                    staysFix[j] = (permutationSet[i][j] == j);
                    isIdentityPerm &= staysFix[j];
                    firstNotFix = staysFix[j] ? firstNotFix : j;
                }

                unsigned int singleLoop[N] = { 0 };
                if (!isIdentityPerm) {
                    unsigned int currentIdx = firstNotFix;
                    for (unsigned int j = 0; j < N; j++) {
                        singleLoop[currentIdx] = 1;
                        currentIdx = permutationSet[i][currentIdx];
                    }
                }

                unsigned int isLoopOrFix = 1;
                for (unsigned int j = 0; j < N; j++) {
                    isLoopOrFix &= (singleLoop[j] ^ staysFix[j]);
                }
                isSingleLoopPerm[i] = !isIdentityPerm && isLoopOrFix;
                hasSingleLoop |= isLoopOrFix;
            }
        }
        assume (hasSingleLoop);

        // check the permutation set whether it is a subset of {\sigma^1, ..., \signma^N}
        unsigned int singleLoopPermIdx = nondet_uint();
        assume (singleLoopPermIdx < permSetSize);
        assume (isSingleLoopPerm[singleLoopPermIdx]);

        struct narray generatedPermSet[MAX_PERM_SET_SIZE] = { { .arr = { 0 } } };
        struct narray currPerm = { .arr = { 0 } };
        struct narray idPerm = { .arr = { 0 } };
        struct narray singleLoopPermNArray = { .arr = { 0 }};
        for (unsigned int i = 0; i < N; i++) {
            idPerm.arr[i] = i;
            singleLoopPermNArray.arr[i] = permutationSet[singleLoopPermIdx][i];
        }
        for (unsigned int i = 0; i < MAX_PERM_SET_SIZE; i++) {
            if (i < permSetSize) {
                struct narray prevPerm = (i == 0) ? idPerm : generatedPermSet[i - 1];
                generatedPermSet[i] = combinePermutations(prevPerm, singleLoopPermNArray);
            }
        }

        unsigned int generatedPermIdx = nondet_uint();
        assume (generatedPermIdx < permSetSize);
        for (unsigned int i = 0; i < MAX_PERM_SET_SIZE; i++) {
            if (i < permSetSize) {
                for (unsigned int j = 0; j < N; j++) {
                    assume (permutationSet[i][j] == generatedPermSet[generatedPermIdx].arr[j]);
                }
            }
        }
    }
...
int main() {
    ...
    for (unsigned int i = 0; i < NUMBER_START_SEQS; i++) {
        ...
        if (WEAK_SECURITY != 2) { // We differentiate the output (i.e., NOT output possibilistic)
            ...
            // MODIFIED
            unsigned int outputOne = isZeroOne(start[i].arr) || isOneZero(start[i].arr);
            inputPoss = 1;
            pos = outputOne; // shorthand of "pos = outputOne ? 1 : 0;"
        }
